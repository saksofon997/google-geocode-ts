...
import { InvalidRequestError, ApiKeyError, NetworkError } from "./errors.js";
  ...
  results: [
    {
      ...
      geometry: {
        location: { lat: 37.422_476_4, lng: -122.084_249_9 },
...
describe("Geocoder", () => {
  ...
  describe("constructor", () => {
    it("should throw if API key is missing", () => {
      assert.throws(() => new Geocoder({ apiKey: "" }), InvalidRequestError);
    });
  ...
  describe("geocode", () => {
    it("should geocode an address", async () => {
      mockFetch.mock.mockImplementation(
        async () => new Response(JSON.stringify(mockGeocodeResponse))
      );
      ...
      assert.strictEqual(results[0].geometry.location.lat, 37.422_476_4);
    ...
    it("should return empty array when no results", async () => {
      ...
      const results = await geocoder.geocode({
        address: "nonexistent address xyz123",
      });
    ...
    it("should throw ApiKeyError for quota exceeded", async () => {
      ...
      await assert.rejects(
        async () => geocoder.geocode({ address: "test" }),
        ApiKeyError
      );
    });
  ...
  describe("reverseGeocode", () => {
    it("should reverse geocode coordinates", async () => {
      ...
      const results = await geocoder.reverseGeocode({
        latlng: { lat: 37.422_476_4, lng: -122.084_249_9 },
      });
    ...
    it("should throw InvalidRequestError for invalid coordinates", async () => {
      await assert.rejects(
        async () => geocoder.reverseGeocode({ latlng: { lat: 91, lng: 0 } }),
        InvalidRequestError
      );

      await assert.rejects(
        async () => geocoder.reverseGeocode({ latlng: { lat: 0, lng: 181 } }),
        InvalidRequestError
      );
    });
  ...
  describe("getCoordinates", () => {
    it("should return coordinates for an address", async () => {
      ...
      assert.strictEqual(coords.lat, 37.422_476_4);
      assert.strictEqual(coords.lng, -122.084_249_9);
    });
    ...
    it("should return null when no results", async () => {
      ...
      const coords = await geocoder.getCoordinates(
  ...
  describe("getAddress", () => {
    it("should return address for coordinates", async () => {
      ...
      const address = await geocoder.getAddress({
        lat: 37.422_476_4,
        lng: -122.084_249_9,
      });
